...
float x_unit[2]={1,0};//预定义单位向量
float b[2];
Eigen::Vector3d p_past = {0,0,0};
double yaw_past;
double yaw_temp;
bool VK_YAW;
bool FIX_HGT;
//余弦定理
float yaw_calcu(float a[2], float b[2]) //输入当前航点的xy坐标和目标航点的xy，我们只做二维的yaw运算
{
    float ab, a1, b1, cosr, r;
    ab = a[0] * b[0] + a[1] * b[1];
...
//尝试在AUTO_HOVER和CMD_CTRL之间添加一个节点把yaw角在仿真器允许的范围内变到正确朝向
/* 
...
{	
	...
	case AUTO_HOVER:
	{
		...
		{
			if (state_data.current_state.mode == "OFFBOARD")//当前状态为offboard_mode
			{
				state = CMD_CTRL;//切换state
				des = get_cmd_des();
...
Desired_State_t PX4CtrlFSM::get_sharphold_des()
{
	Desired_State_t des;
	des.p = hover_pose.head<3>();
	des.v = Eigen::Vector3d::Zero();
...
Desired_State_t PX4CtrlFSM::get_cmd_des()
{
	...
	if(VK_YAW)
	{
	//modified yaw calculation
	b[0] = des.p(0) - p_past(0);//无人机运动朝向解算 未来位置-当前位置
    b[1] = des.p(1) - p_past(1);
	ROS_INFO("past position: %f, %f", p_past(0),p_past(1));
	ROS_INFO("current position: %f, %f", des.p(0), des.p(1));
	//ROS_INFO("desire yaw vector:%f, %f", b[0],b[1]);
    des.yaw = yaw_calcu(b, x_unit);
	if(b[1]<0)
    {
        des.yaw = - des.yaw;
    }
	...
	while (fabs(des.yaw - yaw_past) >= 1.5)
	{
		yaw_temp = (des.yaw-yaw_past)*0.5 + yaw_past;
		get_sharphold_des();
		//des.p = p_past;
		//sharp holding, how to write
	}
	//ROS_INFO("YAW GAP:%f", fabs(des.yaw - yaw_past));
	p_past(0) = des.p(0);
	p_past(1) = des.p(1);
	p_past(2) = des.p(2);
	yaw_past = des.yaw;                                                                   
