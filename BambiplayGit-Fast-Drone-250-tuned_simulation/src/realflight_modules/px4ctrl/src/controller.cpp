...
using namespace std;
Eigen::Vector3d Pei(0.0, 0.0, 0.0), Vei(0.0, 0.0, 0.0);
    ...
    Controller_Output_t &u)
{
  /* WRITE YOUR CODE HERE */
      ...
      Eigen::Vector3d Kp,Kv,Kpi,Kvi;
      
      Kp << param_.gain.Kp0, param_.gain.Kp1, param_.gain.Kp2;
      Kv << param_.gain.Kv0, param_.gain.Kv1, param_.gain.Kv2;
      //Kpi << 0.1, 0.1, 0.0;
      //Kvi << 0.0, 0.0, 0.0;
      Pei = Pei + des.p - odom.p;
      Vei = Vei + des.v - odom.v;
      des_acc = des.a + Kv.asDiagonal() * (des.v - odom.v) + Kp.asDiagonal() * (des.p - odom.p);
      /*             + Kpi.asDiagonal()*Pei               + Kvi.asDiagonal()*Vei;*/
    ...
    const Eigen::Vector3d &des_acc)
{
  ...
  //ROS_INFO("present percentage: %f", throttle_percentage);//used for checking the hovering percentage
...
LinearControl::resetThrustMapping(void)
{
  ...
  P_ = 1e6;
  ROS_INFO("hover_percentage: %f", param_.thr_map.hover_percentage);
}
